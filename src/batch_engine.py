...
# def run_batch_processing(input_file, id_run="batch_run_raw_01"):
#     print("\n" + "="*60)
#     print(f"🚀 بدء معالجة الباتش (Python Batch) مع ضمان عدم التكرار الحقيقي (Hash Idempotency)")
#     print(f"📁 الملف: {input_file}")
#     print(f"🔑 رقم التشغيل: {id_run}")
#     print("="*60)
    
#     client = get_mongo_client()
#     db = client[settings.DB_NAME]
    
#     # 1. إنشاء فهرس فريد على row_hash لضمان عدم تكرار أي سطر نهائياً
#     try:
#         db[settings.RAW_COLLECTION].create_index(
#             [("row_hash", 1)], 
#             unique=True, 
#             name="unique_row_hash_idx"
#         )
#         print("✅ تم تفعيل فهرس منع التكرار الفريد (Unique Hash Index) بنجاح.")
#     except Exception as e:
#         print(f"⚠️ ملاحظة الفهرس: {e}")
...
#                 # توليد بصمة فريدة (Hash) لكل صف بناءً على محتواه بالكامل
#                 row_string = json.dumps(row, sort_keys=True)
#                 row_hash = hashlib.md5(row_string.encode('utf-8')).hexdigest()
...
#                 # تطبيق Upsert بالاعتماد على البصمة الفريدة للسجل
#                 res = db[settings.RAW_COLLECTION].update_one(
#                     {"row_hash": row_hash},
#                     {"$set": raw_record},
#                     upsert=True
#                 )
...
#         os.makedirs(settings.REPORTS_DIR, exist_ok=True)
#         report_path = os.path.join(settings.REPORTS_DIR, "batch_results.json")
#         with open(report_path, "w", encoding="utf-8") as rep_file:
#             json.dump(metrics, rep_file, ensure_ascii=False, indent=4)
            
#         print(f"✅ تم الانتهاء بنجاح تام!")
#         print(f"📊 إجمالي السجلات المقروءة في الملف: {metrics['total_rows']}")
#         print(f"📥 السجلات الجديدة (Inserted): {metrics['inserted_count']}")
#         print(f"🔄 السجلات المحدثة بدون تكرار (Updated): {metrics['updated_count']}")
#         print(f"⏱️ الوقت المستغرق: {metrics['elapsed_seconds']} ثانية")
#         print("="*60)
        
#     except Exception as e:
#         print(f"❌ حدث خطأ أثناء معالجة الباتش: {e}")
#     finally:
#         client.close()

# if __name__ == "__main__":
#     run_batch_processing(settings.SAMPLE_DATA_FILE, "run_batch_hash_idempotent_001")